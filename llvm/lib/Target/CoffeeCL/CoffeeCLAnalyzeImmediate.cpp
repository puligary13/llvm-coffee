//===-- CoffeeCLAnalyzeImmediate.cpp - Analyze Immediates ---------------------===//
//
...
CoffeeCLAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}

// Add I to the instruction sequences.
void CoffeeCLAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void CoffeeCLAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void CoffeeCLAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void CoffeeCLAnalyzeImmediate::GetInstSeqLsSLL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void CoffeeCLAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void CoffeeCLAnalyzeImmediate::ReplaceADDiuSLLWithLUi(InstSeq &Seq) {
...
void CoffeeCLAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const CoffeeCLAnalyzeImmediate::InstSeq
&CoffeeCLAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
                                 bool LastInstrIsADDiu) {
    this->Size = Size;

    ADDiu = CoffeeCL::ADDiu;
    ORi = CoffeeCL::ORi;
    SLL = CoffeeCL::SLL;
    LUi = CoffeeCL::LUi;
