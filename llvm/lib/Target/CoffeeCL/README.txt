//===- README.txt - Notes for improving CoffeeCL-specific code gen ---------===//
...
implement a CoffeeCL fastcc, where we could do whatever we wanted to the CC, 
...
logical op, it will invoke CoffeeCLInstrInfo::convertToThreeAddress.  At this point
...
Is compile to this with the CoffeeCL (32-bit) backend:
...
transformation, good for PI.  See CoffeeCLISelLowering.cpp, this comment:
...
test/CodeGen/CoffeeCL/2007-03-24-cntlzd.ll compiles to:
...
We generate horrible CoffeeCL code for this:
...
; RUN: llvm-as < %s | llc -march=CoffeeCL32  | grep or | count 3
; This should produce one 'or' or 'cror' instruction per function.

; RUN: llvm-as < %s | llc -march=CoffeeCL32  | grep mfcr | count 3
; PR2964
...
//===----------------------------------------------------------------------===//
; RUN: llvm-as < %s | llc -march=CoffeeCL32 | not grep fneg
