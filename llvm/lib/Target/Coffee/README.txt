//===- README.txt - Notes for improving Coffee-specific code gen ---------===//
...
implement a Coffee fastcc, where we could do whatever we wanted to the CC, 
...
logical op, it will invoke CoffeeInstrInfo::convertToThreeAddress.  At this point
...
Is compile to this with the Coffee (32-bit) backend:
...
transformation, good for PI.  See CoffeeISelLowering.cpp, this comment:
...
test/CodeGen/Coffee/2007-03-24-cntlzd.ll compiles to:
...
We generate horrible Coffee code for this:
...
; RUN: llvm-as < %s | llc -march=Coffee32  | grep or | count 3
; This should produce one 'or' or 'cror' instruction per function.

; RUN: llvm-as < %s | llc -march=Coffee32  | grep mfcr | count 3
; PR2964
...
//===----------------------------------------------------------------------===//
; RUN: llvm-as < %s | llc -march=Coffee32 | not grep fneg
