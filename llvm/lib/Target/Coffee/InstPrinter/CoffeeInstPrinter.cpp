...
const char* Coffee::CoffeeFCCToString(Coffee::CondCode CC) {
  switch (CC) {
  case FCOND_F:
...
static void printExpr(const MCExpr *Expr, raw_ostream &OS) {
  ...
  case MCSymbolRefExpr::VK_Coffee_ABS_HI:    OS << "%hi(";     break;
  case MCSymbolRefExpr::VK_Coffee_ABS_LO:    OS << "%lo(";     break;
...
void CoffeeInstPrinter::printUnsignedImm(const MCInst *MI, int opNum,
                                       raw_ostream &O) {
...
printFCCOperand(const MCInst *MI, int opNum, raw_ostream &O) {
  const MCOperand& MO = MI->getOperand(opNum);
  O << CoffeeFCCToString((Coffee::CondCode)MO.getImm());
}
